...
class AddTriggersToImportantTables extends Migration
{
    ...
    public function up()
    {
        ...
        DB::unprepared('CREATE TRIGGER post_company_update AFTER UPDATE ON `companies` FOR EACH ROW
                BEGIN
                   INSERT INTO `companies_revs` (`id`, `name`, `owner_id`, `description`, `active_status`, `ps_live_secret`, `ps_test_secret`, `created_at`, `updated_at`,`deleted_at`, `action`) 
                   VALUES (OLD.id, OLD.name, OLD.owner_id, OLD.description, OLD.active_status, OLD.ps_live_secret, OLD.ps_test_secret, OLD.created_at, OLD.updated_at, OLD.deleted_at, "update");
                END');
        DB::unprepared('CREATE TRIGGER post_company_delete AFTER DELETE ON `companies` FOR EACH ROW
                BEGIN
                   INSERT INTO `companies_revs` (`id`, `name`, `owner_id`, `description`, `active_status`, `ps_live_secret`, `ps_test_secret`, `created_at`, `updated_at`,`deleted_at`, `action`) 
                   VALUES (OLD.id, OLD.name, OLD.owner_id, OLD.description, OLD.active_status, OLD.ps_live_secret, OLD.ps_test_secret, OLD.created_at, OLD.updated_at, OLD.deleted_at, "delete");
                END');

        //Employees update and delete trigger
        ...
        //Locations update and delete trigger
        ...
        DB::unprepared('CREATE TRIGGER post_order_update AFTER UPDATE ON `orders` FOR EACH ROW
                BEGIN
                   INSERT INTO `orders_revs` (`id`,`user_id`,`order_type`,`services`,`status`,`amount`,`payment_method`,`created_by`,`location_id`,`company_id`,`collected`,`note`,`bags`, `created_at`,`updated_at`, `deleted_at`, `action`) 
                   VALUES (OLD.id, OLD.user_id, OLD.order_type, OLD.services, OLD.status, OLD.amount, OLD.payment_method, OLD.created_by, OLD.location_id, OLD.company_id, OLD.collected, OLD.note, OLD.bags, OLD.created_at, OLD.updated_at, OLD.deleted_at, "update");
                END');
        DB::unprepared('CREATE TRIGGER post_order_delete AFTER DELETE ON `orders` FOR EACH ROW
                BEGIN
                   INSERT INTO `orders_revs` (`id`,`user_id`,`order_type`,`services`,`status`,`amount`,`payment_method`,`created_by`,`location_id`,`company_id`,`collected`,`note`,`bags`, `created_at`,`updated_at`, `deleted_at`, `action`) 
                   VALUES (OLD.id, OLD.user_id, OLD.order_type, OLD.services, OLD.status, OLD.amount, OLD.payment_method, OLD.created_by, OLD.location_id, OLD.company_id, OLD.collected, OLD.note, OLD.bags, OLD.created_at, OLD.updated_at, OLD.deleted_at, "delete");
                END');
